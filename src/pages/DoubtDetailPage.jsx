  ...
  return (
    <div className="card" style={{ padding: 24, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 600, color: '#1A1D23' }}>
          {isTeacher ? '🏫 Post Your Answer' : '💬 Your Answer'}
        </h3>
        {isTeacher && (
          <span style={{ fontSize: '0.75rem', color: '#6B7280', background: '#F3F4F6', padding: '3px 10px', borderRadius: 999 }}>
            Visible to student
          </span>
        )}
      </div>
        ...
        {tabs.map(({ key, icon: Icon, label }) => (
          <button
            ...
            style={{
              ...
              color: activeTab === key ? '#6B7CC4' : '#6B7280',
      ...
      {/* Media preview */}
      {mediaPreview && (
        <div style={{ marginBottom: 14, borderRadius: 10, overflow: 'hidden', border: '1px solid #E5E7EB', position: 'relative' }}>
          {/* Remove button */}
          <button
            onClick={clearMedia}
            style={{
          ...
          {mediaType === 'image' && (
            <>
              ...
              />
              <div style={{ padding: '8px 14px', background: '#F9FAFB', borderTop: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', gap: 6 }}>
                <ImagePlus size={13} color="#7BAE8F" />
          ...
          {mediaType === 'video' && (
            <>
              ...
              />
              <div style={{ padding: '8px 14px', background: '#1A1D23', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Video size={13} color="#7BAE8F" />
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              background: mediaType === 'image' ? '#F0FDF4' : 'white',
              borderColor: mediaType === 'image' ? '#86EFAC' : '#E5E7EB',
              color: mediaType === 'image' ? '#15803D' : '#6B7280',
          ...
          <button
            ...
            style={{
              ...
              borderColor: mediaType === 'video' ? '#BAE6FD' : '#E5E7EB',
              color: mediaType === 'video' ? '#0369A1' : '#6B7280',
              transition: 'all 0.15s',
            }}
          >
            <Video size={14} />
...
export default function DoubtDetailPage() {
  ...
  if (!doubt) return (
    <div style={{ textAlign: 'center', padding: 80 }}>
      <p style={{ color: '#9CA3AF' }}>Doubt not found.</p>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FA', padding: '24px 20px' }}>
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <div className="card" style={{ padding: 28, marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {doubt.is_anonymous ? (
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#F3F4F6', border: '2px dashed #D1D5DB', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <EyeOff size={18} color="#9CA3AF" />
                </div>
              ) : <AvatarInitials name={displayName} size={40} />}
              <div>
                <p style={{ margin: 0, fontSize: '0.9rem', fontWeight: 600, color: '#374151' }}>{displayName}</p>
                <p style={{ margin: 0, fontSize: '0.78rem', color: '#9CA3AF' }}>{timeAgo(doubt.created_at)}</p>
              </div>
            </div>
          ...
          <h1 style={{ margin: '0 0 12px', fontSize: '1.25rem', fontWeight: 700, color: '#1A1D23', lineHeight: 1.4 }}>
            {doubt.title}
          </h1>
          <p style={{ margin: '0 0 16px', fontSize: '0.9rem', color: '#4B5563', lineHeight: 1.75, whiteSpace: 'pre-wrap' }}>
            {doubt.description}
          </p>
          ...
          {doubt.image_url && (
            <div style={{ marginBottom: 16, borderRadius: 10, overflow: 'hidden', border: '1px solid #E5E7EB' }}>
              <img
                ...
                style={{ width: '100%', maxHeight: 350, objectFit: 'contain', background: '#F9FAFB', cursor: 'pointer' }}
          ...
          {/* Actions bar */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 14, borderTop: '1px solid #F3F4F6', flexWrap: 'wrap' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: '0.85rem', color: '#6B7280' }}>
              <MessageSquare size={15} /> {answers.length} {answers.length === 1 ? 'answer' : 'answers'}
            </span>
            {!isOwner && (
              <button onClick={() => setShowReport(!showReport)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 7, border: '1px solid #E5E7EB', background: 'white', color: '#9CA3AF', fontSize: '0.8rem', cursor: 'pointer' }}>
                <Flag size={13} /> Report
              </button>
            )}
            {(isOwner || isTeacher) && (
              <button onClick={handleDeleteDoubt} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 7, border: '1px solid #FECACA', background: '#FEF2F2', color: '#DC2626', fontSize: '0.8rem', cursor: 'pointer' }}>
          ...
          {showReport && (
            <div style={{ marginTop: 14, padding: 16, background: '#FEF2F2', borderRadius: 10, border: '1px solid #FECACA' }}>
              <p style={{ margin: '0 0 10px', fontSize: '0.85rem', fontWeight: 600, color: '#DC2626' }}>Report this doubt</p>
        ...
        <div style={{ marginBottom: 20 }}>
          <h2 style={{ margin: '0 0 16px', fontSize: '1rem', fontWeight: 700, color: '#1A1D23', display: 'flex', alignItems: 'center', gap: 8 }}>
