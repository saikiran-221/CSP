  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F9FA', padding: '28px 20px' }}>
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            <h1 style={{ margin: '0 0 4px', fontSize: '1.5rem', fontWeight: 700, color: '#1A1D23', display: 'flex', alignItems: 'center', gap: 10 }}>
              <Shield size={22} color="#6B7CC4" /> Moderation Panel
            </h1>
            <p style={{ margin: 0, color: '#6B7280', fontSize: '0.9rem' }}>Review and act on reported content</p>
          </div>
        ...
        {reports.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 10, padding: '10px 16px', marginBottom: 20 }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {reports.map(r => (
              <div key={r.id} className="card" style={{ padding: 22, borderLeft: '4px solid #F59E0B' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 14, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Flag size={16} color="#F59E0B" />
                    <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#92400E', background: '#FFFBEB', padding: '2px 10px', borderRadius: 999 }}>
                      Reported Doubt
                    </span>
                    <span style={{ fontSize: '0.75rem', color: '#9CA3AF' }}>{timeAgo(r.created_at)}</span>
                  </div>
                </div>

                {r.doubts && (
                  <div style={{ background: '#F9FAFB', borderRadius: 9, padding: '12px 16px', marginBottom: 14 }}>
                    <p style={{ margin: '0 0 4px', fontSize: '0.875rem', fontWeight: 600, color: '#1A1D23' }}>{r.doubts.title}</p>
                    <p style={{ margin: 0, fontSize: '0.8rem', color: '#6B7280', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      {r.doubts.description}
                    </p>
                    <Link to={`/doubt/${r.target_id}`} style={{ display: 'inline-block', marginTop: 8, fontSize: '0.78rem', color: '#6B7CC4', textDecoration: 'none', fontWeight: 500 }}>
                      View full doubt →
                    </Link>
                  </div>
                )}
